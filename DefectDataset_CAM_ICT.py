...
class DefectDataset(DataLoader):
    ...
    def transforms(self, image, target = None):
        ...
        # if random.random() > 0.5:
        #     colorJitter = transforms.ColorJitter(brightness = [0.75, 1.05], 
        #     contrast = [0.75, 1.75], saturation = [0.85, 1.15])
        #     image = colorJitter(image)
        # if random.random() > 0.5:
        #     angle,_,scale,shear = transforms.RandomAffine.get_params(degrees = [-5,5], 
        #     translate = None, scale_ranges = (1.0, 1.0), shears = (0,0), 
        #     img_size = (224,224))
